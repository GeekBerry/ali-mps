 ...
 * 提交转码作业
 *
 * @description 提交转码作业接口，一个转码输出会生成一个转码作业，接口返回转码作业列表。
 * 作业会添加到管道中被调度执行，执行完成后需要调用查询转码作业接口轮询作业执行结果，也可使用异步通知机制。
 * (无论提交还是查询, 一次都只支持一个任务)
 *
 * @see https://help.aliyun.com/document_detail/29226.html
 * @see https://help.aliyun.com/document_detail/29253.html 参数详情
 */
class SubmitJobsQuery extends QueryBase {
  constructor(options) {
   ...
   * @param remove {Boolean} 是否删除源文件, 默认为 false
   * @return {SubmitJobsQuery}
   */
  audio({ codec, channels, bitrate, samplerate, profile, remove }) {
...
/**
 * 列出转码作业
 * @description 通过作业状态，创建时间区间，转码管道列出转码作业，默认按CreationTime降序排列。
 *
 * @see https://help.aliyun.com/document_detail/29229.html
 */
class ListJobQuery extends QueryBase {
  constructor(options) {
 ...
 * @see https://help.aliyun.com/document_detail/29228.html
 */
class QueryJobQuery extends QueryBase {
  constructor(options) {
...
class CancelJobQuery extends QueryBase {
  constructor(options) {
