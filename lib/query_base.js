...
class QueryBase {
  ...
  signatureQuery() {
    const query = {
      ...
      SignatureNonce: Date.now() + lodash.random(0, 10000).toString(),
      Timestamp: moment().utc().format('YYYY-MM-DD[T]HH:mm:ss[Z]'),

      ...QueryBase.serialize(this.query), // serialize 不会改变 this.query
    };

    const canonicalized = objToUri(sortKeys(query));
    const stringToSign = `${this.method}&${encodeURIComponent('/')}&${encodeURIComponent(canonicalized)}`;
    query.Signature = sha1Base64(stringToSign, `${this.accessKeySecret}&`); // 注意: 密钥后加 '&'
   ...
   */
  request() {
    return superagent(this.method, this.endpoint).query(this.signatureQuery());
  }
